...
**Important:** New skills are sandboxed by default (`disable-model-invocation: true`). The user must explicitly enable them by editing the manifest and running `/skill-tree:sync`.
...
After adding, suggest running `/skill-tree:sync` to update cluster routing tables.
